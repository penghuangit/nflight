 ...
 * @see ExitAllServer
 */
public class ExitAllServer{
	
	private static final Class<ExitAllServer> THIS_CLAZZ = ExitAllServer.class;
	...
	public static void main(String[] args){
		...
		// 설정파일(nflight_system.properties)을 시스템프로퍼티에 반영합니다.
				boolean bl = PropertyLoader.load(StartAllServer.NFLIGHT_SYSTEM_PROPERTY_NAME,
						StartAllServer.NFLIGHT_SYSTEM_CONFIG_FILE_NAME);
				if (bl == false) {
				...
				bl = PropertyLoader.load(StartAllServer.NFLIGHT_PROPERTY_NAME,
						StartAllServer.NFLIGHT_CONFIG_FILE_NAME);
				if (bl == false) {
					LOGGER.logp(Level.SEVERE, THIS_CLAZZ.getName(), METHOD_NAME,
				...
				if (os == null) {
					os = StartAllServer.NFLIGHT_DEFAULT_OS;
				}
