...
public class SocketAcceptorTest {
    ...
    private static void testDatagramAcceptor(InetAddress DEFAULT_ADDRESS, int DEFAULT_PORT) throws Exception {

	startDatagramAcceptor(DEFAULT_ADDRESS, DEFAULT_PORT);
	Thread.sleep(1000);
	// startDatagramAcceptorClient(DEFAULT_ADDRESS, DEFAULT_PORT, new
	// Message("shutdown"));
	startDatagramAcceptorClient(DEFAULT_ADDRESS, DEFAULT_PORT, "shutdown");
    }
	...
	DatagramAcceptorClient client = new DatagramAcceptorClient(DEFAULT_ADDRESS, DEFAULT_PORT);
    ...
    private static void startDatagramAcceptor(InetAddress DEFAULT_ADDRESS, int DEFAULT_PORT) throws Exception {

	AcceptorWorker worker = new AcceptorWorker();
	new Thread(worker).start();
	new Thread(new DatagramAcceptorImpl(DEFAULT_ADDRESS, DEFAULT_PORT, worker)).start();

    }

    private static void testStreamAcceptor(InetAddress DEFAULT_ADDRESS, int DEFAULT_PORT) {
	try {
	    startStreamAcceptor(DEFAULT_ADDRESS, DEFAULT_PORT);
	    Thread.sleep(1000);
	    startStreamAcceptorClient(DEFAULT_ADDRESS, DEFAULT_PORT, new Message("shutdown"));
	} catch (IOException | InterruptedException e) {
	    e.printStackTrace();
	}
    }

    private static void startStreamAcceptor(InetAddress DEFAULT_ADDRESS, int DEFAULT_PORT) throws IOException {

	AcceptorWorker worker = new AcceptorWorker();
	new Thread(worker).start();
	new Thread(new StreamAcceptorImpl(DEFAULT_ADDRESS, DEFAULT_PORT, worker)).start();
	...
	StreamAcceptorClient client = new StreamAcceptorClient(DEFAULT_ADDRESS, DEFAULT_PORT);
	Thread t = new Thread(client);
