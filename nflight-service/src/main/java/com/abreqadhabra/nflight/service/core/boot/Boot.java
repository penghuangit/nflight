...
public class Boot {
	...
	public static Properties parseServiceSpecifiers(Properties props) {
		...
		String service = props
				.getProperty(Env.Boot.KEY_BOOT_OPTION_SERVICE);
		...
		props.setProperty(Env.Boot.KEY_BOOT_OPTION_SERVICE_NAME,
				serviceName);
		props.setProperty(Env.Boot.KEY_BOOT_OPTION_SERVICE_CLASS,
				serviceClass);
		props.setProperty(
				Env.Boot.KEY_BOOT_OPTION_SERVICE_COMMAND,
				serviceCommand);
	...
	public static void main(String[] args) {
	    ...
	    String serviceName = System
		    .getProperty(Env.Boot.KEY_BOOT_OPTION_SERVICE_NAME);

	    String serviceClass = System
		    .getProperty(Env.Boot.KEY_BOOT_OPTION_SERVICE_CLASS);
	    ...
	    if (serviceName
		    .equals(Env.Boot.STR_SERVICE_NAME_RMI_CLIENT)) {

	    } else if (serviceName
		    .equals(Env.Boot.STR_SERVICE_NAME_RMI_SERVER)) {
		/*
		 * parameterTypeList.add(BootProfileImpl.class);
		 * initArgList.add(profile);
		 */
	    } else if (serviceName
		    .equals(Env.Boot.STR_SERVICE_NAME_SOCKET_CLIENT)) {

	    } else if (serviceName
		    .equals(Env.Boot.STR_SERVICE_NAME_SOCKET_SERVER)) {

	    } else if (serviceName
		    .equals(Env.Boot.STR_SERVICE_NAME_DATA_SERVER)) {
...
/*
	...
	try {
		...
		if (key.startsWith("--")) {
		    ...
		    if (key.equalsIgnoreCase(Env.Boot.VAL_BOOT_OPTION_GUI)) {
			argsProps.setProperty(
				Env.Boot.KEY_BOOT_OPTION_GUI, value);
		    } else {
			argsProps
				.setProperty(
					Env.Boot.KEY_BOOT_OPTION_GUI,
					"false");
		    }

		    if (key.equalsIgnoreCase(Env.Boot.VAL_BOOT_OPTION_SERVICE)) {
			value = (String) options.next();
			if (checkNotLeadingHyphens(key, value)) {
			    argsProps.setProperty(
				    Env.Boot.KEY_BOOT_OPTION_SERVICE,
				    value);
			}
		    } else if (key
			    .equalsIgnoreCase(Env.Boot.VAL_BOOT_OPTION_HOST)) {
			value = (String) options.next();
			if (checkNotLeadingHyphens(key, value)) {
			    argsProps.setProperty(
				    Env.Boot.KEY_BOOT_OPTION_HOST,
				    value);
			}
		    } else if (key
			    .equalsIgnoreCase(Env.Boot.VAL_BOOT_OPTION_PORT)) {
			value = (String) options.next();
			if (checkNotLeadingHyphens(key, value)) {
			    argsProps.setProperty(
				    Env.Boot.KEY_BOOT_OPTION_PORT,
				    value);
			}
		    } else if (key
			    .equalsIgnoreCase(Env.Boot.VAL_BOOT_OPTION_CONF)) {
