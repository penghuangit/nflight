...
public class PropertyLoader
{
	 ...
	 * @exception IOException 입출력 에러가 발생한 경우
	     * @since STEP1
	     */
...
/*	public static boolean load(String propertyName) throws IOException {
		...
		try {
			URL propertiesLocation = getLocation(propertyName);
			if (propertiesLocation == null) {
				throw new FileNotFoundException("Not found configuration file");
			}
			propertiesInput = propertiesLocation.openStream();
...
/*
	...
	/**
	 * [機　能] このプロパティリストの保存先を URL で返します。
	 * [説　明] CONFIGURATION_PROPERTY キーに対する値のファイル名から、
	 * その位置を URL で返します。
	 * ファイルの位置を変更する場合は、起動時にコマンドラインオプション
	 * -D"CONFIGURATION_PROPERTYキー名"="ファイル名" で変更してください。
	 * [備　考] なし。
	 */
/*	public static URL getLocation(String configurationPropertyName) {
		URL location = null;
		if (location == null) {
			location = PropertyLoader.getResource(configurationPropertyName);
		}
		return location;
	}*/

	/**
	 * [機　能] 指定された名前のリソースを検索して返します。
	 * [説　明] 以下の順に指定された名前のリソースを検索して返します。
	 * <ol>
	 * <li>指定された名前が URL として解釈できる場合は、
	 * そのまま URL に変換して返します。
	 * リソースの存在は確認しません。
	 * <li>指定された名前をファイルパスの直接参照として検索します。
	 * ファイルまたはディレクトリが存在する場合は、
	 * その絶対パスを URL に変換して返します。
	 * <li>指定された名前をこのクラスのパッケージ位置で検索します。
	 * リソースが見つかった場合は、その URL を返します。
	 * <li>指定された名前をシステムクラスパスから検索します。
	 * リソースが見つかった場合は、その URL を返します。
	 * <li>指定された名前のリソースが見つからない場合は、
	 * null を返します。
	 * </ol>
	 * [備　考] なし。
	 * @param name リソースの名前
...
/*	public static URL getResource(String name) {
		...
		resourceURL = PropertyLoader.class.getResource(name);
		if (resourceURL != null) {
			return resourceURL;
		}
		resourceURL = ClassLoader.getSystemResource(name);
		if (resourceURL != null) {
			return resourceURL;
		}
		return null;
